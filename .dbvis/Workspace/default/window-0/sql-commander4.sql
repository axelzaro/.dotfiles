SELECT
    ...
    ('//configuration/triggers/triggerDefinition/pluginKey/text()').getStringVal() 
FROM
    build_definition BD
JOIN
    BUILD B
...
WHERE
    XMLTYPE(BD.XML_DEFINITION_DATA).EXTRACT
    ('//configuration/triggers/triggerDefinition/pluginKey/text()').getStringVal() IS NOT NULL
AND XMLTYPE(BD.XML_DEFINITION_DATA).EXTRACT
